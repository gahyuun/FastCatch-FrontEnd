...
import { filterStateTypes } from "@/states/filterState";

interface locationProps {
  allocated: [string, string];
  onChangeSelected: React.Dispatch<
    React.SetStateAction<"location" | "date" | "amount" | null>
  >;
}
...
const LocationCategoryName = (props: locationProps) => {
  ...
  const selectionHandler = (event: React.MouseEvent) => {
    ...
    setFilterStates(prevStates => {
      return {
        ...prevStates,
        locale: props.allocated[0] as filterStateTypes["locale"],
      };
    });
  ...
  return (
    <div
      className={
        props?.allocated[0] === filterStates.locale ? "selected" : "unSelected"
      } //
